...
The template collects device availability, HTTP API availability, Dune player state, playback progress, playback position, playback duration, playback speed, buffering status, audio state, firmware/app versions, serial information, and provides a ready-to-use dashboard.
...
- Is the player in standby, navigator, playback, Blu-ray playback, or black screen mode?
- Is media playback currently active?
- What is the playback position and duration?
- What is the playback progress percentage?
- Is playback buffering?
- What is the current playback speed?
- What is the volume and mute state?
- Which firmware and Dune app version are installed?
- What are the product name, product ID, and serial numbers?
...
- `dune.playback_progress_pct` returns `0%` when playback progress cannot be calculated, because JavaScript preprocessing in Zabbix may not expose the same custom-on-fail handling as JSONPath preprocessing.
- `dune.android_ui_active` is not an availability metric. It can be `0` during native media playback while the device is working normally.
...
- The information dashboard widget uses mostly static inventory-like items. Some of them use a `1d` discard-unchanged heartbeat, so a very short dashboard time range may temporarily hide rows until the next heartbeat.

---

## Screenshots

![Dune overview dashboard](docs/dashboard-dune-overview.png)
...
```text
templates/template_dune_hd_by_http.yaml
```
...
|---|---|---|---|
| Dune: ICMP ping | Simple check | `icmpping` | `0/1` |
| Dune: HTTP API availability | Simple check | `net.tcp.service[http,,{$DUNE.API.PORT}]` | `0/1` |
| Dune: Status raw | HTTP agent | `dune.status.raw` | JSON text |
| Dune: Protocol version | Dependent | `dune.protocol_version` | number |
| Dune: Product name | Dependent | `dune.product_name` | text |
| Dune: Product ID | Dependent | `dune.product_id` | text |
| Dune: Firmware version | Dependent | `dune.firmware_version` | text |
| Dune: App version | Dependent | `dune.app_version` | text |
| Dune: Serial number | Dependent | `dune.serial_number` | text |
| Dune: Commercial serial number | Dependent | `dune.commercial_serial_number` | text |
| Dune: Android UI active | Dependent | `dune.android_ui_active` | mapped enum |
| Dune: Player state | Dependent | `dune.player_state` | text |
| Dune: Player state code | Dependent | `dune.player_state.code` | mapped enum |
| Dune: Playback speed | Dependent | `dune.playback_speed` | speed code |
| Dune: Playback duration | Dependent | `dune.playback_duration` | seconds |
| Dune: Playback position | Dependent | `dune.playback_position` | seconds |
| Dune: Playback progress | Dependent | `dune.playback_progress_pct` | `%` |
| Dune: Playback buffering | Dependent | `dune.playback_is_buffering` | mapped enum |
| Dune: Playback DVD menu | Dependent | `dune.playback_dvd_menu` | mapped enum |
| Dune: Playback volume | Dependent | `dune.playback_volume` | `%` |
| Dune: Playback mute | Dependent | `dune.playback_mute` | mapped enum |
...
This prevents old standby/non-playback periods from polluting graph min/avg values with artificial `-1` values.
...
### Row 4: Audio and timeline

- Audio state graph
- Playback position and duration graph

### Row 5: Information and problems
...
When no movie is playing, the player may report:

```text
player_state = navigator / standby
```
...
`Android UI` may show `UI inactive (0)` during playback. This is expected and should not be considered a problem.

---

## Notes about Dune Android UI state
...
During testing, this value behaved like a Dune/Android UI shell flag rather than a health metric:

```text
1 = UI active
0 = UI inactive, often during native playback
```
...
### Dune information widget shows “No data found”

The information widget uses static inventory-like values such as product name, firmware version, and serial number. These values use a `1d` discard-unchanged heartbeat.

If the dashboard time range is too short immediately after import, increase the dashboard time range to:

```text
Last 24 hours
```

or wait until the next heartbeat update.

---

### Android UI shows inactive during playback

This is expected. The value may switch to `0` while native playback is active.

Do not treat this item as a problem unless your own use case specifically requires the Android UI shell to stay active.
...
- Optional support for additional content metadata if exposed by the Dune status API
