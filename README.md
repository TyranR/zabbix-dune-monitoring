...
- What was the last/current playback title?
...
Version **1.1** adds advanced playback metadata, including now-playing information, bitrate, resolution, selected audio/subtitle data, playback URL, and last playback tracking.
...
- The information dashboard widget uses mostly static inventory-like items. Some of them may be hidden with a very short dashboard time range until the next heartbeat/history sample.
...
|---|---|---|---|---:|
| Dune: ICMP ping | Simple check | `icmpping` | 0/1 or number | 7d |
| Dune: HTTP API availability | Simple check | `net.tcp.service[http,,{$DUNE.API.PORT}]` | 0/1 or number | 7d |
...
| Item | Type | Key | Units / Value | History |
|---|---|---|---|---:|
| Dune: Android UI active | Dependent | `dune.android_ui_active` | 0/1 or number | 7d |
| Dune: Player state | Dependent | `dune.player_state` | text | 7d |
| Dune: Player state code | Dependent | `dune.player_state.code` | 0/1 or number | 7d |
...
| Dune: Playback buffering | Dependent | `dune.playback_is_buffering` | 0/1 or number | 7d |
| Dune: Playback DVD menu | Dependent | `dune.playback_dvd_menu` | 0/1 or number | 7d |
| Dune: Playback volume | Dependent | `dune.playback_volume` | % | 7d |
| Dune: Playback mute | Dependent | `dune.playback_mute` | 0/1 or number | 7d |
| Dune: Playback current bitrate | Dependent | `dune.playback_current_bitrate` | bps | 90d |
| Dune: Is video | Dependent | `dune.is_video` | 0/1 or number | 90d |
| Dune: Scrambling detected | Dependent | `dune.scrambling_detected` | 0/1 or number | 90d |
...
| Dune: Current subtitles track index | Dependent | `dune.subtitles_track` | 0/1 or number | 90d |
...
When playback stops, the preprocessing discards new non-playback values, so the item keeps the last time when playback was still observed.
...
### Dashboard duplication if placed in Item history

Dynamic playback items should not be placed inside the `Dune information` widget if that widget uses the `Item history` type. `Item history` shows historical rows, not just the latest value.

This can create repeated lines such as:

```text
Last played title
Last playback ended at
Last played title
Last playback ended at
...
```

For this reason, the dashboard uses separate **Item value** widgets for:
...
If the value is shifted by a few hours, check the timezone of the relevant container:
...
For Docker deployments, set the appropriate timezone, for example:
...
### Playback speed or bitrate has gaps on the graph
...
The information widget uses static inventory-like values such as product name, firmware version, and serial number. These values may use a discard-unchanged heartbeat.
...
or wait until the next heartbeat/history sample.
